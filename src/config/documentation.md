...
This step must be completed before connecting to our AVB network via a Digiface AVB, as it replaces the default Digiface AVB configuration generated by RME AVB Controller with the specific configuration required for our network.

<mark>Note</mark> This provides an alternative to RME's default configuration method, which requires users to manually change the AVB protocol mode and configure all AVB stream formats and clocking routes. If you encounter errors while following the steps below, [use this method instead](<>).
